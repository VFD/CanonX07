...
8 '- Pierre BARTHE -
10 GOSUB10000
15 CLS:CONSOLE0,4,0,0,1:CONSOLE@,1
17 FONT$(131)="48,48,48,48,48,48,48,48"
20 CLS:CLEAR:PI=3.1415926535098#
...
100 Q=STICK(0)
105 IFA$="0"ANDZ=HTHENV=V-12
110 H=10000*INT(X/G)+INT(Y/G)
120 H=H+N:H=H*N+PI:H=1000*H*PI
...
320 IFV>75ORV<65THEN360
330 R=-Q+D
340 V=V-.5
350 Z=Z+R:W=C*12/V:GOTO420
360 R=Q+D
370 IFV<75ANDZ-N=0THENR=0
...
617 PRINTUSING"######";Y
620 LOCATE 0,3
630 IFZ-H>0THENPRINTCHR$(132);ELSEPRINTCHR$(133);
633 LOCATE 13,2
637 PRINTUSING"###";B
640 LOCATE 1,2
650 PRINTUSING"###";V
...
1180 RETURN
2000 FORI=0TO10:CLS:LOCATE 7,1:PRINT"CRASH !!!!":NEXTI
2005 A=STICK(1)
2020 CLS:PRINT:PRINT"UNE AUTRE PARTIE?":PRINT"     (O/N)"
2030 A$=INKEY$:IFA$=""THEN2030ELSEIFA$="O"THEN10ELSEIFA$<>"N"THEN2030
2035 FORI=1TO50:BEEP5*I,2:BEEP3*I,5:NEXT
...
5040 PRINT" Am    ";CHR$(223);CHR$(131);"Vz     m/s";
5050 LINE(98,24)-(119,24)
5060 RETURN
10000 CLS
10010 LOCATE 5,0:PRINT"SIMULATEUR":LOCATE 9,1:PRINT"D E":LOCATE 9,2:PRINT"VOL";
...
10180 CLS:PRINT"    Voulez-vous ":PRINT" le mode d'emploi ?        (O/N)";
10190 A$=INKEY$:IFA$=""THEN10190ELSEIFA$="N"THENRETURNELSEIFA$<>"O"THEN10190
10195 CLS:CONSOLE0,3
10200 CLS:PRINT" Ceci est un simula- teur de vol aux     instruments....";
10210 LOCATE10,3:FORI=248TO240STEP-1:PRINTCHR$(I);:NEXT
10220 PSET(119,31):PSET(117,31):PSET(115,31)
10230 A$=INKEY$:IFA$=""THEN10230
10239 CLS
10240 PRINT"  Ce Jeu n'a pas de":PRINT"  but precis , mais" :PRINT" il donne un bon...";
10245 A$=INKEY$:IFA$=""THEN10245
10250 CLS:PRINT" apercu des techni-":PRINT" ques du pilotage":PRINT" sans visibilite";
...
10270 CLS:PRINT"---MODE D'EMPLOI---":PRINT"Vous pilotez un ":PRINT"avion de tourisme..";
10275 A$=INKEY$:IFA$=""THEN10275
10280 CLS:PRINT" De 9m d'envergure":PRINT"De vitesse de decro-  chage : 75 km/h";
10285 A$=INKEY$:IFA$=""THEN10285
10290 CLS:PRINT" Les touches du cur-seur definissent la":PRINT" position du manche";
10295 A$=INKEY$:IFA$=""THEN10295
10300 CLS:PRINT" >: a droite":PRINT" ^: pousse":PRINT"<^: pousse a gauche";
10305 A$=INKEY$:IFA$=""THEN10305
10310 CLS:PRINT" La manette des gaz":PRINT"est represente par":PRINT"les touches 1 -> 9";
10315 A$=INKEY$:IFA$=""THEN10315
10320 CLS:PRINT"1->5 pour ralentir":PRINT"6->  sans objet":PRINT"7->9 pour accelerer";
10325 A$=INKEY$:IFA$=""THEN10325
10327 CLS:PRINT" 0 sert a freiner."
10328 A$=INKEY$:IFA$=""THEN10328
10330 CLS:PRINT" La touche centrale   du curseur sert   a mettre l'avion en";
...
10350 CLS:PRINT"<Alt>:altitude(m)","<Cap>:cap(";CHR$(223);")","<Vi> :vitesse(km/h)";
10351 A$=INKEY$:IFA$=""THEN10351
10352 CLS:PRINTCHR$(132);":l'avion est en vol";CHR$(133);":l'avion est pose";
...
10400 CLS:PRINT" Le nombre en bas a  droite represente   le fuel restant";
...
10430 CLS:PRINT"Le terrain est divi-se en carres de 500mde cote chacun...";
